...
TEST_CASE("negpar.unit.numerics.kinetic Euler fluxes handle periodic and "
	...
	REQUIRE(coulomb::Numerics{}.limited_flux(
...
TEST_CASE("negpar.unit.moments.moment conversions round trip", "[moments]") {
	...
	REQUIRE_THROWS_AS(
		coulomb::MomentOperations{}.compute_primitive(
...
TEST_CASE("negpar.unit.resampling.shared resampling numerics map modes and "
	...
	REQUIRE(coulomb::resampling::ResamplingNumerics{}.augmented_locations(
				4, 2) == std::vector<std::size_t>{0, 1, 2, 7});
	REQUIRE(coulomb::resampling::ResamplingNumerics{}.imaginary_frequencies(
...
#pragma omp parallel num_threads(thread_count)
		{
			...
			for (int draw_index = 0; draw_index < draws_per_thread;
				 ++draw_index) {
				const auto index =
					static_cast<std::size_t>(thread) * draws_per_thread +
					draw_index;
...
TEST_CASE("negpar.unit.output.macro output writer preserves numbered precision",
	...
	REQUIRE_THROWS_AS(
		coulomb::ParticleOutput{}.save_homogeneous_radial_distribution(0,
																	   state),
		std::invalid_argument);
