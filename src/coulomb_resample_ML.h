...
void interp3d_fft(NeParticleGroup * S_x, complex<double> *Fouriercoeff, int Nfreq1, int Nfreq2, int Nfreq3) {
  ...
  // double Neff = Neff_temp * Lxyz[0]*Lxyz[1]*Lxyz[2]/cubic_2pi; // need to multiply para.Neff, here we take para.Neff = 1
...
void interp3d_fft_approx_terms(complex<double> *Fouriercoeff, double *f, int Nfreq1, int Nfreq2, int Nfreq3, int augFactor, int orderx, int ordery, int orderz) {
...
void filter_Fourier(complex<double> * Fouriercoeff, int * flag_Fouriercoeff, int size_FC) {
  // double thres = 10.0;
...
double interp3d_fvalue(double* Sf, complex<double> * Fouriercoeff, complex<double> *ifreq1, complex<double> *ifreq2, complex<double> *ifreq3, int * flag_Fouriercoeff, int Nfreq1, int Nfreq2, int Nfreq3) {
...
void interp3d_acceptsampled(double* Sf, Particle1d3d *Sp_incell, Particle1d3d *Sn_incell, 
                            double fval, double &maxf, double Neff_cell, 
                            int &Nskx, int &kskx, int &kp_incell, int &kn_incell) {
  ...
  if (abs(fval)>maxf) {
    ...
    while (kkkk < kp_incell) {
      if (myrand()>keeprate) {
        double *Sf_last = (Sp_incell + kp_incell - 1) -> velocity();
    ...
    while (kkkk < kn_incell) {
      if (myrand()>keeprate) {
        double *Sf_last = (Sn_incell + kn_incell - 1) -> velocity();
...
*/
void interp3d_fcoarse(complex<double> * Fouriercoeff, double *fcoarse, int Nfreq1, int Nfreq2, int Nfreq3) {
...
void interp3d_fxyz_terms(complex<double> *Fouriercoeff, double *f, int Nfreq1, int Nfreq2, int Nfreq3, int augFactor, int orderx, int ordery, int orderz) {
...
void interp3d_fxyz(complex<double> * Fouriercoeff, double *f, double *fx, double *fy, double *fz, 
                  double *fxx, double *fyy, double *fzz, double *fxy, double *fxz, double *fyz, 
    ...
    for (int ky = 0; ky < augFactor*Nfreq; ky ++ ) {
      for (int kz = 0; kz < augFactor*Nfreq; kz ++ ) {
        ...
        // sum_Nk += Nskx;
          ...
          // double fval = interp3d_fvalue(Sf, Fouriercoeff, ifreq, ifreq, ifreq, flag_Fouriercoeff, Nfreq, Nfreq, Nfreq);
          ...
          interp3d_acceptsampled(Sf, Sp_incell, Sn_incell, fval, maxf, Neff/(dxaug*dxaug*dxaug), Nskx, kskx, kp_incell, kn_incell);

  				if ( kskx >= Nskx ) {
						flag_conti = false;
            for (int kp_c = 0; kp_c < kp_incell; kp_c ++) {
              (Sp_sampled + kp + kp_c) -> set_velocity( (Sp_incell+kp_c) -> velocity());
            }
            for (int kn_c = 0; kn_c < kn_incell; kn_c ++) {
              (Sn_sampled + kn + kn_c) -> set_velocity( (Sn_incell+kn_c) -> velocity());
            }              
						kp += kp_incell;
						kn += kn_incell;
  				}
...
void particleresample_inhomo_nocoll(NeParticleGroup * S_x, NumericGridClass & grid, ParaClass & para, MultlLevelGroup * MLsol) {
  if ( ( count_particle_number(S_x, grid.Nx, 'p') + count_particle_number(S_x, grid.Nx, 'n') ) >= (10000*grid.Nx) ) {
    for (int kx = 0; kx < grid.Nx; kx ++) {
      cout << "Particles resampling: ( "  <<  (S_x+kx)->size('p') << ", "
                                          <<  (S_x+kx)->size('n') << ", "
                                          <<  (S_x+kx)->size('f') << ") " << endl;
...
void interp3d_fft_ml(complex<double> *Fouriercoeff, int * flag_Fouriercoeff, MultlLevelGroup * MLsol, int Nlevel) {
  ...
  int Nfine = 1<<Nlevel;
  for (int kk = 0; kk < Nfine*Nfine*Nfine; kk++) Fouriercoeff[kk] = complex<double> (0., 0.);
