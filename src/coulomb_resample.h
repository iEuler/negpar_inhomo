...
void interp_freq(vector<double>& freq, int Nfreq) {
  for (int j=0; j<Nfreq; j++) 
    freq[j] = (double) (freq_sequence(j, Nfreq));    
}
...
void interp3d_fft(NeParticleGroup * S_x, complex<double> *Fouriercoeff, int Nfreq1, int Nfreq2, int Nfreq3) {
  ...
  // the (i,j,k)-th element of the array with size (Nx,Ny,Nz), you would use the expression an_array[k + Nz * (j + Ny * i)].
    ...
    for (int kk2 = 0; kk2 < Nfreq2; kk2 ++) {
      for (int kk3 = 0; kk3 < Nfreq3; kk3 ++) {
        ...
        for (int kp=0; kp<Np; kp++) {
          ...
          complex<double> expterm = vp[0]*ifreq1[kk1] + vp[1]*ifreq2[kk2] + vp[2]*ifreq3[kk3];
...
void interp3d_fft_approx_terms(complex<double> *Fouriercoeff, vector<double>& f, int Nfreq1, int Nfreq2, int Nfreq3, int augFactor, int orderx, int ordery, int orderz) {
  ...
  plan3d_ft = fftw_plan_dft_3d(augFactor*Nfreq1, augFactor*Nfreq2, augFactor*Nfreq3, fin, FSaug, FFTW_FORWARD, FFTW_ESTIMATE);
  ...
  for (int kk1 = 0; kk1 < Nfreq1; kk1 ++) {
    for (int kk2 = 0; kk2 < Nfreq2; kk2 ++) {
      for (int kk3 = 0; kk3 < Nfreq3; kk3 ++) {
        ...
        int kk2aug = loc2[kk2];
        int kk3aug = loc3[kk3];
        int kkaug = kk3aug + augFactor*Nfreq3 * (kk2aug + augFactor*Nfreq2 * kk1aug);
        ...
        if ((orderx+ordery+orderz)==2) {
          if ( (orderx == 2) || (ordery == 2) || (orderz == 2) ) freq *= -.5;
          else freq *= -1;
        } 
  
        if ((orderx+ordery+orderz)==1) {
          Fouriercoeff[kk] += freq * complex<double> (FSaug[kkaug][1], -FSaug[kkaug][0]);
        } else {
          Fouriercoeff[kk] += freq * complex<double> (FSaug[kkaug][0], FSaug[kkaug][1]);
        } 
...
void interp3d_fft_approx(NeParticleGroup * S_x, complex<double> *Fouriercoeff, int Nfreq1, int Nfreq2, int Nfreq3) {
  ...
  // for the (i,j,k)-th element of the array with size (Nx,Ny,Nz), use the expression an_array[k + Nz * (j + Ny * i)].
  ...
  for (int kk = 0; kk < Nfreq1*Nfreq2*Nfreq3; kk++) Fouriercoeff[kk] = complex<double> (0., 0.);
    
  interp3d_fft_approx_terms(Fouriercoeff, f,   Nfreq1, Nfreq2, Nfreq3, augFactor, 0, 0, 0);
  
  interp3d_fft_approx_terms(Fouriercoeff, fx,  Nfreq1, Nfreq2, Nfreq3, augFactor, 1, 0, 0);
  interp3d_fft_approx_terms(Fouriercoeff, fy,  Nfreq1, Nfreq2, Nfreq3, augFactor, 0, 1, 0);
  interp3d_fft_approx_terms(Fouriercoeff, fz,  Nfreq1, Nfreq2, Nfreq3, augFactor, 0, 0, 1);
  interp3d_fft_approx_terms(Fouriercoeff, fxx, Nfreq1, Nfreq2, Nfreq3, augFactor, 2, 0, 0);
  interp3d_fft_approx_terms(Fouriercoeff, fyy, Nfreq1, Nfreq2, Nfreq3, augFactor, 0, 2, 0);
  interp3d_fft_approx_terms(Fouriercoeff, fzz, Nfreq1, Nfreq2, Nfreq3, augFactor, 0, 0, 2);
  interp3d_fft_approx_terms(Fouriercoeff, fxy, Nfreq1, Nfreq2, Nfreq3, augFactor, 1, 1, 0);
  interp3d_fft_approx_terms(Fouriercoeff, fxz, Nfreq1, Nfreq2, Nfreq3, augFactor, 1, 0, 1);
  interp3d_fft_approx_terms(Fouriercoeff, fyz, Nfreq1, Nfreq2, Nfreq3, augFactor, 0, 1, 1);  
  
  for (int kk = 0; kk < Nfreq1*Nfreq2*Nfreq3; kk++) Fouriercoeff[kk] *= coeff_fft;
...
void filter_Fourier(complex<double> * Fouriercoeff, vector<int>& flag_Fouriercoeff, int size_FC) {
  // double thres = 10.0;
...
double interp3d_fvalue(double* Sf, complex<double> * Fouriercoeff, complex<double> *ifreq1, complex<double> *ifreq2, complex<double> *ifreq3, vector<int>& flag_Fouriercoeff, int Nfreq1, int Nfreq2, int Nfreq3) {
    ...
    for (int kk2 = 0; kk2 < Nfreq2; kk2 ++ ) {
      for (int kk3 = 0; kk3 < Nfreq3; kk3 ++ ) {          
        ...
        if ( flag_Fouriercoeff[kkk]	>0 ) { 
          fval_c += Fouriercoeff[kkk]*exp( ifreq1[kk1]*Sf[0] + ifreq2[kk2]*Sf[1] + ifreq3[kk3]*Sf[2] );
        }
      }
    }
  }
...
double interp3d_fvalue_approx(double deltax, double deltay, double deltaz, 
      ...
      double fxx, double fyy, double fzz, double fxy, double fxz, double fyz) {
  
  double f0;
  f0 = f + fx*deltax + fy*deltay + fx*deltay 
      + .5*fxx*deltax*deltax + .5*fyy*deltay*deltay + .5*fzz*deltaz*deltaz
      + fxy*deltax*deltay + fxz*deltax*deltaz + fyz*deltay*deltaz;
  return f0;
}


void interp3d_acceptsampled(double* Sf, NeParticleGroup* ptr_S_x_incell, double fval, double &maxf) {
  ...
  if (abs(fval)>maxf) {
	...
	if (myrand()<(abs(fval/maxf))) {
		...
		for (int kv =0; kv<3; kv ++) sum_Sf_pi_sq += (Sf[kv]-pi)*(Sf[kv]-pi);
...
void resampleF_acceptsampled(double* Sf, NeParticleGroup* ptr_S_x_incell, double fval, double &maxf) {
...
*/
void interp3d_fcoarse(complex<double> * Fouriercoeff, vector<double>& fcoarse, int Nfreq1, int Nfreq2, int Nfreq3) {

  // double Lcubic = (double) (Nfreq1*Nfreq2*Nfreq3);

  fftw_complex *FC, a_fftw_complex_variable, *fcoarse_c;
  FC = (fftw_complex*) fftw_malloc(Nfreq1*Nfreq2*Nfreq3 * sizeof(a_fftw_complex_variable));
  fcoarse_c = (fftw_complex*) fftw_malloc(Nfreq1*Nfreq2*Nfreq3 * sizeof(a_fftw_complex_variable));
  ...
  plan3d_ift = fftw_plan_dft_3d(Nfreq1, Nfreq2, Nfreq3, FC, fcoarse_c, FFTW_BACKWARD, FFTW_ESTIMATE);
...
void interp3d_fxyz_terms(complex<double> *Fouriercoeff, vector<double>& f, int Nfreq1, int Nfreq2, int Nfreq3, int augFactor, int orderx, int ordery, int orderz) {
  ...
  interp3d_fcoarse(FSaug, f, augFactor*Nfreq1, augFactor*Nfreq2, augFactor*Nfreq3);
...
void interp3d_fxyz(complex<double> * Fouriercoeff, vector<double>& f, vector<double>& fx, vector<double>& fy, vector<double>& fz, 
                  vector<double>& fxx, vector<double>& fyy, vector<double>& fzz, vector<double>& fxy, vector<double>& fxz, vector<double>& fyz, 
                  int Nfreq1, int Nfreq2, int Nfreq3, int augFactor) {
  
  interp3d_fxyz_terms(Fouriercoeff, f,   Nfreq1, Nfreq2, Nfreq3, augFactor, 0, 0, 0);
  interp3d_fxyz_terms(Fouriercoeff, fx,  Nfreq1, Nfreq2, Nfreq3, augFactor, 1, 0, 0);
  interp3d_fxyz_terms(Fouriercoeff, fy,  Nfreq1, Nfreq2, Nfreq3, augFactor, 0, 1, 0);
  interp3d_fxyz_terms(Fouriercoeff, fz,  Nfreq1, Nfreq2, Nfreq3, augFactor, 0, 0, 1);
  interp3d_fxyz_terms(Fouriercoeff, fxx, Nfreq1, Nfreq2, Nfreq3, augFactor, 2, 0, 0);
  interp3d_fxyz_terms(Fouriercoeff, fyy, Nfreq1, Nfreq2, Nfreq3, augFactor, 0, 2, 0);
  interp3d_fxyz_terms(Fouriercoeff, fzz, Nfreq1, Nfreq2, Nfreq3, augFactor, 0, 0, 2);
  interp3d_fxyz_terms(Fouriercoeff, fxy, Nfreq1, Nfreq2, Nfreq3, augFactor, 1, 1, 0);
  interp3d_fxyz_terms(Fouriercoeff, fxz, Nfreq1, Nfreq2, Nfreq3, augFactor, 1, 0, 1);
  interp3d_fxyz_terms(Fouriercoeff, fyz, Nfreq1, Nfreq2, Nfreq3, augFactor, 0, 1, 1);
}
...
// void interp3d_fft_eachlevel(NeParticleGroup * S_x, MultlLevelGroup * MLsol, int Nlevel);
// void interp3d_fft_ml(complex<double> *Fouriercoeff, int * flag_Fouriercoeff, MultlLevelGroup * MLsol, int Nlevel);
...
// void samplefromfourier3d(NeParticleGroup * S_x, NeParticleGroup * S_x_new, MultlLevelGroup * MLsol, int Nlevel) {
void samplefromfourier3d(NeParticleGroup * S_x, NeParticleGroup * S_x_new, int Nfreq) {
  ...
  filter_Fourier(Fouriercoeff, flag_Fouriercoeff, Nfreq*Nfreq*Nfreq); // Apply the filter on Fourier coefficients    
  ...
  interp3d_fxyz(Fouriercoeff, f, fx, fy, fz, fxx, fyy, fzz, fxy, fxz, fyz, Nfreq, Nfreq, Nfreq, augFactor);
  ...
  for (int kx = 0; kx<Nfreq*augFactor; kx++) interp_xaug[kx] = kx*2*pi/Nfreq/augFactor;
    ...
    for (int ky = 0; ky < augFactor*Nfreq; ky ++ ) {
      for (int kz = 0; kz < augFactor*Nfreq; kz ++ ) {
        ...
        NeParticleGroup* ptr_S_x_incell = &S_x_incell;
          ...
          double fval = 0;
          if (flag_useApproximation)
            fval = interp3d_fvalue_approx(deltax, deltay, deltaz, f[kk], fx[kk], fy[kk], fz[kk], 
              fxx[kk], fyy[kk], fzz[kk], fxy[kk], fxz[kk], fyz[kk]);
          else
            fval = interp3d_fvalue(Sf, Fouriercoeff, ifreq, ifreq, ifreq, flag_Fouriercoeff, Nfreq, Nfreq, Nfreq);

          // reset current cell if fval>maxf, otherwise continue sampling in current cell					
        ...
        merge_NeParticleGroup(S_x_new, ptr_S_x_incell);        
  ...
  Particle1d3d *Sp_sampled = S_x_new -> list('p');
  Particle1d3d *Sn_sampled = S_x_new -> list('n');  
  double *xyz_minmax = S_x -> xyz_minmax;
...
void sampleF(NeParticleGroup * S_x, double Neff_F_new, double Neff_F_old) {
  ...
  // // cout << "Resample now " <<  Nf_new << ' ' << Nf_old << endl;
    ...
    for (int kv = 0; kv < 3; kv ++) vmod[kv] = (uf_new[kv] - uf_old[kv])/Neff_F_new/Nf_new;
    ...
    for (int kf = 0; kf < Nf_old; kf ++) {
      double *vf = (Sfold+kf) -> velocity();
      for (int kv = 0; kv < 3; kv ++) Told[kv] += (vf[kv] - c[kv]) * (vf[kv] - c[kv]);
    }    
    for (int kv = 0; kv < 3; kv ++) Told[kv] *= Neff_F_old;
    
    for (int kf = 0; kf < Nf_new; kf ++) {
      double *vf = (Sf+kf) -> velocity();
      for (int kv = 0; kv < 3; kv ++) Tnew[kv] += (vf[kv] - c[kv]) * (vf[kv] - c[kv]);
    }    
    ...
    for (int kf = 0; kf < Nf_new; kf ++) {
      double *vf = (Sf+kf) -> velocity();
      for (int kv = 0; kv < 3; kv ++) vf[kv] = c[kv] + sigma[kv]* (vf[kv] - c[kv]);
      (Sf+kf) -> set_velocity(vf);
    }
...
void sampleF_inhomo(NeParticleGroup * S_x, NumericGridClass & grid, ParaClass & para) {
...
void addMaxwellian_terms(double rhoM, vector<double> uM, vector<double> TM, double Neff, vector<double>& f, int Nfreq, int augFactor, int orderx, int ordery, int orderz) {
  ...
  for (int kx = 0; kx < augFactor*Nfreq; kx ++ ) {
    for (int ky = 0; ky < augFactor*Nfreq; ky ++ ) {
      for (int kz = 0; kz < augFactor*Nfreq; kz ++ ) {
        ...
        if (orderx == 2) Mcc *= ((xc-uM[0])*(xc-uM[0]) - TM[0])/TM[0]/TM[0];
        if (ordery == 1) Mcc *= -(yc-uM[1])/TM[1];
        if (ordery == 2) Mcc *= ((yc-uM[1])*(yc-uM[1]) - TM[1])/TM[1]/TM[1];
        if (orderz == 1) Mcc *= -(zc-uM[2])/TM[2];
        if (orderz == 2) Mcc *= ((zc-uM[2])*(zc-uM[2]) - TM[2])/TM[2]/TM[2];
...
void addMaxwellian(double rhoM, vector<double> uM, vector<double> TM, double Neff, 
                  vector<double>& f, vector<double>& fx, vector<double>& fy, vector<double>& fz, 
                  vector<double>& fxx, vector<double>& fyy, vector<double>& fzz, 
                  vector<double>& fxy, vector<double>& fxz, vector<double>& fyz, 
...
void resample_F_from_MPN(NeParticleGroup * S_x, NeParticleGroup * S_x_new, int Nfreq, double Neff, double Neff_F, double dx_space) {
  ...
  addMaxwellian(rhoM, uM, TM, Neff, f, fx, fy, fz, fxx, fyy, fzz, fxy, fxz, fyz, Nfreq, augFactor);
    ...
    for (int ky = 0; ky < augFactor*Nfreq; ky ++ ) {
      for (int kz = 0; kz < augFactor*Nfreq; kz ++ ) {
        ...
        NeParticleGroup* ptr_S_x_incell = &S_x_incell;
          ...
          // compute f at this point
          double fval = interp3d_fvalue_approx(deltax, deltay, deltaz, f[kk], fx[kk], fy[kk], fz[kk], 
            fxx[kk], fyy[kk], fzz[kk], fxy[kk], fxz[kk], fyz[kk]);
        ...
        mergeF_NeParticleGroup(S_x_new, ptr_S_x_incell);        
  ...
  // rescale to the original coordinates
  Particle1d3d *Sp_sampled = S_x_new -> list('f');
  double *xyz_minmax = S_x -> xyz_minmax;
...
// void particleresample_homo(NeParticleGroup * S_x, const ParaClass & para, MultlLevelGroup * MLsol) {
bool particleresample_homo(NeParticleGroup * S_x, const ParaClass & para) {
  ...
  int Np_new = ptr_S_x_new -> size('p'), Nn_new = ptr_S_x_new -> size('n');
  
  // cout << " Resample finished." << endl;
  // cout << "After resampling N = (" << ptr_S_x_new ->size('p') << ", " << ptr_S_x_new ->size('n') << ");" << endl;
...
void save_homo_dist(NeParticleGroup * S_x, const NumericGridClass & grid, int flag_case);
void resampleF_inhomo(NeParticleGroup * S_x, double Neff_F_new, NumericGridClass & grid, int Nfreq);

// void particleresample_inhomo(NeParticleGroup * S_x, NumericGridClass & grid, ParaClass & para, MultlLevelGroup * MLsol) {
void particleresample_inhomo(NeParticleGroup * S_x, NumericGridClass & grid, ParaClass & para) {
  ...
  if (needGlobalResample) {
    ...
    for (int kx = 0; kx < grid.Nx; kx ++) {
      resample_spatial_ratio += ( (S_x+kx)->size('p') + (S_x+kx)->size('n') ) / ( (S_x+kx)->size('f') );
    }
    ...
    while ((flag_resample_success) && (kx < grid.Nx)) {
      if ( ( (S_x+kx)->size('p') + (S_x+kx)->size('n') ) >= resample_spatial_ratio*(S_x+kx)->size('f') ) {
  ...
  if (!flag_resample_success) {
    ...
    for (int kx=0; kx<Nx; kx++) 	rho[kx] = (S_x+kx)-> rhoM
                                         + ( (S_x+kx)->m0P - (S_x+kx)->m0N) * grid.Neff / grid.dx;
  
    for (int kx=0; kx<Nx; kx++) 	rho_F[kx] = (S_x+kx)-> m0F * grid.Neff_F / grid.dx;
...
void resampleF_homo(NeParticleGroup * S_x, double Neff_F_new,  double Neff, int Nfreq, double dx_space) {
  
  NeParticleGroup S_x_new;
  NeParticleGroup *ptr_S_x_new = & S_x_new;
  
  // resample particles
  resample_F_from_MPN(S_x, ptr_S_x_new, Nfreq, Neff, Neff_F_new, dx_space);
  ...
  S_x -> clear('f');
  mergeF_NeParticleGroup(S_x, ptr_S_x_new);    
...
void resampleF_keeptotalmass(NeParticleGroup * S_x, NumericGridClass & grid, int Nf_old) {
...
void sync_coarse(NeParticleGroup * S_x, NumericGridClass & grid, ParaClass & para) {
  if (para.flag_collision == 1) {
    if (SYNC_TIME>para.sync_time_interval) {
      ...
      int Nd = 0;
      for (int kx = 0; kx < grid.Nx; kx ++)  Nd += ( (S_x+kx) -> size('p') + (S_x+kx) -> size('n') );
      int Nf = (int)(1.2*Nd);    
