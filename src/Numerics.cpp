...
double Numerics::error_function(double x) {
...
Numerics::central_difference(const std::vector<double> &values, int size,
  ...
  const auto left = boundary == BoundaryCondition::Periodic
                        ? Numerics::circular_shift(values, size, -1)
                        : Numerics::boundary_shift(values, size, -1, values[0]);
  const auto right =
      boundary == BoundaryCondition::Periodic
          ? Numerics::circular_shift(values, size, 1)
          : Numerics::boundary_shift(values, size, 1, values[size - 1]);
...
Numerics::limited_flux(const std::vector<double> &values, int size, double dx,
  ...
  const auto left =
      boundary == BoundaryCondition::Periodic
          ? Numerics::circular_shift(values, size, -1)
          : Numerics::boundary_shift(values, size, -1, boundary_values.front());
  const auto right =
      boundary == BoundaryCondition::Periodic
          ? Numerics::circular_shift(values, size, 1)
          : Numerics::boundary_shift(values, size, 1, boundary_values.back());
...
void Numerics::advance_kinetic_euler(
  ...
  for (int index = 0; index < size; ++index) {
    ...
    const double positive_fraction =
        0.5 *
        (1.0 + Numerics::error_function(velocity[index] /
  ...
  const auto d1_positive = Numerics::limited_flux(g1_positive, size, dx, dt, 1,
                                                  g1_positive, boundary);
  const auto d1_negative = Numerics::limited_flux(g1_negative, size, dx, dt, -1,
                                                  g1_negative, boundary);
  const auto d2_positive = Numerics::limited_flux(g2_positive, size, dx, dt, 1,
                                                  g2_positive, boundary);
  const auto d2_negative = Numerics::limited_flux(g2_negative, size, dx, dt, -1,
                                                  g2_negative, boundary);
  const auto d3_positive = Numerics::limited_flux(g3_positive, size, dx, dt, 1,
                                                  g3_positive, boundary);
  const auto d3_negative = Numerics::limited_flux(g3_negative, size, dx, dt, -1,
