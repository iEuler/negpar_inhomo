...
void save_macro(const vector<T>& macro, string filename) {
  ...
  if (FLAG_FILENAME_WITH_NUM)	filename_mod = "result/" + filename + int2str(K_SAVE_TIME) + subfix;
  else filename_mod = "result/" + filename + subfix;
  
  // if (FLAG_FILENAME_WITH_NUM)	filename_mod = filename + int2str(K_SAVE_TIME) + subfix;
  // else filename_mod = filename + subfix;
...
void save_2d(int Nrow, int Ncol, const vector<vector<double>> & data2d, string filename) {
  string filename_mod, subfix = ".txt";
...
void save_rhouT(int Nx, const vector<double>& rho, const vector<double>& u1, 
        const vector<double>& u2, const vector<double>& u3, const vector<double>& Tprt) {
...
void save_dist(NeParticleGroup * S_x, const NumericGridClass & grid, char partype) {
  int Nx = grid.Nx;  
  int Nbar = grid.Nv;
...
void save_particle1d1d(NeParticleGroup * S_x, const NumericGridClass & grid, char partype, int whattosave) {
...
void save_macro_evolution(NeParticleGroup * ptr_S_x,  const NumericGridClass & grid) {
