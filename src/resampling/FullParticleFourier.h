...
class FullParticleFourier {
  public:
	...
	std::vector<std::vector<double>> interpolate_derivatives(
		const std::vector<std::complex<double>>& coefficients, int frequency_x,
		int frequency_y, int frequency_z, int augmentation_factor);
	std::vector<double> upper_bound(int count,
