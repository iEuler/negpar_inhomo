...
// Global variables
  ...
  clock_t t0_all, t1_all, t0_coll, t1_coll, t0_adve, t1_adve, t0_resamp, t1_resamp;
...
class ParaClass {
	public:
  int FLAG_USE_OPENMP;
  double dt, coeff_binarycoll, resample_ratio, resample_spatial_ratio, sync_time_interval, resample_sync_ratio;
  string method_binarycoll, method; // method = HDP or PIC
  int Npickup_neg, Nfreq, Nlevel, Num_grids, Num_gridpoints;
  // Npickup_neg: number of particles used in the evaluation of neg part of Delta M
  // Nfreq: parameters used in resampling. Number of frequeces in Fourier interpolation
  // Nlevel: parameters used in resampling. Number of levels in Multi Level version of Fourier interpolation
  // Num_grids = (3*Nlevel*Nlevel - 3*Nlevel + 2)/2: parameters used in resampling.
  int flag_collision; // 0 = no collision, 1 = Coulomb collison, 2 = BGK collision
  double lambda_Poisson; // the lambda used in the Poission equation: 1/\lambda * \nabla^2 \phi = \rho

  int flag_resample; // 1 = resample current cell when Nf>=Np+Nn is violated; 2 = resample all cells when it's violated in one cell
...
ParaClass::ParaClass() {
  ...
  Num_gridpoints = ( Nlevel*(Nlevel+1)*(1<<(Nlevel+2)) + Nlevel*(Nlevel-1)*(1<<(Nlevel+1))
                      + (Nlevel-1)*(Nlevel-2)*(1<<Nlevel) ) /2;
  flag_collision = 1; // 0 = no collision, 1 = Coulomb collison, 2 = BGK collision
  lambda_Poisson = 10.0;
...
class IniValClass {
 public:
  ...
  double TSI_alpha, TSI_coe, TSI_rhof, TSI_rhop, TSI_Tprt, TSI_max_f_over_M, TSI_m21, TSI_m22, TSI_m23;
  ...
  //          (1-\beta) * rho0 / |X| * exp(-|x-x_c|^2/2/Tx) * 1/(2*pi*dT)^3/2 * exp(-|v-u_b|^2/2/dT)
...
class NumericGridClass {
 public:
  int Nx, Nt, Nv;
  double xmax, xmin, vmax, vmin, tmax, dx, dv, dt, Neff, Neff_F; // Neff is the effective number
  // double *x, *vx;
  vector<double> x, vx;
  char bdry_x, bdry_v;
  ...
  NumericGridClass(int n_x, string method); // n_x is the number of grid in x direction
...
NumericGridClass::NumericGridClass(int n_x) {  
  NumericGridClass(n_x, "HDP");
}

NumericGridClass::NumericGridClass() {
  NumericGridClass(100);
}
...
void ParticleGroup::push_back(Particle1d3d * Spnew) {
  vS.push_back(*Spnew);  
}
...
class NeParticleGroup {	
 ...
 public:
  ...
  double dx_rhoM, dx_u1M, dx_TprtM; // derivative in x direction, used in sampling from source part
  
  double rho, u1, Tprt; // the moments of all distributions
  // double dx_rho, dx_u1, dx_Tprt; // derivative in x direction, used in sampling from source part
  
  double rho_o, u1_o, Tprt_o; // the moments of all distributions before advection
  
  double drho, dm1, denergy; // change in moments: rho^{n+1} = rho^n - drho
  double drho_g, dm1_g, denergy_g; // change in moments due to g = f_p - f_n, used in sampling from source part
  
  double m0P, m11P, m12P, m13P, m2P, m21P, m22P, m23P, m31P, m32P, m33P; // the moments of P particles, with Neff = 1	
  double m0N, m11N, m12N, m13N, m2N, m21N, m22N, m23N, m31N, m32N, m33N; // the moments of N particles, with Neff = 1	
  double m0F, m11F, m12F, m13F, m2F, m21F, m22F, m23F, m31F, m32F, m33F; // the moments of F particles, with Neff = 1	
  
  double m0P_o, m11P_o, m12P_o, m13P_o, m2P_o, m21P_o, m22P_o, m23P_o, m31P_o, m32P_o, m33P_o; // the moments of P particles before advection, with Neff = 1	
  double m0N_o, m11N_o, m12N_o, m13N_o, m2N_o, m21N_o, m22N_o, m23N_o, m31N_o, m32N_o, m33N_o; // the moments of N particles before advection, with Neff = 1	
  
  double elecfield, elecfield_F;
  
  double alpha_neg, alpha_pos, rmax;
  
  double xyz_minmax[6]; // record the information on [xmin, xmax, ymin, ymax, zmin, zmax]
  ...
  void push_back(const Particle1d3d &, char); // add one more particle in char type
  void erase(int, char); // remove the int particle in char type
  void clear(char); // remove every particle in char type
  
  Particle1d3d * list(char); 
  Particle1d3d list(int, char); 
  void computemoments(); // compute the moments
  void computexyzrange(); // update xyz_minmax = [xmin, xmax, ymin, ymax, zmin, zmax]
...
void NeParticleGroup::computemoments() {
  ...
  m0P = 1.0*Np;
  m11P = 0.0; m12P = 0.0; m13P = 0.0;
  m21P = 0.0; m22P = 0.0; m23P = 0.0;
  m31P = 0.0; m32P = 0.0; m33P = 0.0;
  ...
  for (int kp = 0; kp<Np; kp++) {	
    ...
    m11P += v1;		m12P += v2;		m13P += v3;
    m21P += v1*v1;		m22P += v2*v2;		m23P += v3*v3;
    m31P += v1*vsq;		m32P += v2*vsq;		m33P += v3*vsq;
  }
  ...
  m21N = 0.0; m22N = 0.0; m23N = 0.0;
  m31N = 0.0; m32N = 0.0; m33N = 0.0;
  ...
  for (int kn = 0; kn<Nn; kn++) {	
    ...
    m11N += v1;		m12N += v2;		m13N += v3;
    m21N += v1*v1;		m22N += v2*v2;		m23N += v3*v3;
    m31N += v1*vsq;		m32N += v2*vsq;		m33N += v3*vsq;
  }
  ...
  m0F = 1.0*Nf;
  m11F = 0.0; m12F = 0.0; m13F = 0.0;
  m21F = 0.0; m22F = 0.0; m23F = 0.0;
  m31F = 0.0; m32F = 0.0; m33F = 0.0;
  ...
  for (int kf = 0; kf<Nf; kf++) {	
    ...
    m11F += v1;		m12F += v2;		m13F += v3;
    m21F += v1*v1;		m22F += v2*v2;		m23F += v3*v3;
    m31F += v1*vsq;		m32F += v2*vsq;		m33F += v3*vsq;
...
void NeParticleGroup::copymoments() {
  m0P_o = m0P;   m11P_o = m11P; m12P_o = m12P; m13P_o = m13P; 
  m2P_o = m2P;   m21P_o = m21P; m22P_o = m22P; m23P_o = m23P;
  m31P_o = m31P; m32P_o = m32P; m33P_o = m33P; 
  
  m0N_o = m0N;   m11N_o = m11N; m12N_o = m12N; m13N_o = m13N; 
  m2N_o = m2N;   m21N_o = m21N; m22N_o = m22N; m23N_o = m23N;
  m31N_o = m31N; m32N_o = m32N; m33N_o = m33N; 
  
  rho_o = rho; u1_o = u1; Tprt_o = Tprt;
}
...
Particle1d3d * NeParticleGroup::list(char partype) {
	Particle1d3d * Slist = NULL;
  if (partype == 'p') {
    Slist = &vSp[0];
  } else if (partype == 'n') {
    Slist = &vSn[0];
