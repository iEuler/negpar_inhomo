...
size_t checked_element_count(size_t n1, size_t n2, size_t n3 = 1) {
  ...
  if (plane_count != 0 && n3 > std::numeric_limits<size_t>::max() / plane_count)
    throw std::length_error("FFT buffer size overflows size_t");
...
void FFTWBufferDeleter::operator()(fftw_complex* buffer) const noexcept {
  if (buffer != nullptr) fftw_free(buffer);
}
...
FFT1D::FFT1D(size_t n)
    : n_(n), func_(nullptr), funcFFT_(nullptr), fftPlan_(nullptr),
      ifftPlan_(nullptr) {
  if (n_ == 0) throw std::invalid_argument("FFT1D dimension must be positive");
  const int n_int = fftw_dimension(n_);
  const auto element_count = checked_element_count(n_, 1);
  func_.reset(static_cast<fftw_complex*>(fftw_malloc(
      element_count * sizeof(fftw_complex))));
  funcFFT_.reset(static_cast<fftw_complex*>(fftw_malloc(
      element_count * sizeof(fftw_complex))));
  if (!func_ || !funcFFT_)
...
std::vector<std::complex<double>> FFT1D::fft(const std::vector<double>& func) {
  if (func.size() != n_)
    throw std::invalid_argument("FFT1D::fft input size does not match plan");
...
std::vector<double> FFT1D::ifft(
  ...
  if (funcFFT.size() != n_)
    throw std::invalid_argument("FFT1D::ifft input size does not match plan");
...
FFT3D::FFT3D(size_t n1, size_t n2, size_t n3)
  ...
  fftPlan_.reset(fftw_plan_dft_3d(n1_int, n2_int, n3_int, func_.get(),
                                  funcFFT_.get(), FFTW_FORWARD, FFTW_ESTIMATE));
  ifftPlan_.reset(fftw_plan_dft_3d(n1_int, n2_int, n3_int, funcFFT_.get(),
                                   func_.get(), FFTW_BACKWARD, FFTW_ESTIMATE));
  if (!fftPlan_ || !ifftPlan_)
